...
class ShellEnv(MultiTurnEnv):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        self.seed = seed
        dataset = load_dataset('deathbyknowledge/V3-shell-format', split=['train'])[0] # type: ignore
        eval_dataset = dataset.select(range(200, 300)) # type: ignore
        dataset = dataset.select(range(300, dataset.num_rows)) # type: ignore
    ...
    def format_dataset(self,
        ...
        def format_prompt_fn(example: dict) -> dict:
            ...
            task = prompt[0].get("content")
            info = {}
            setup_commands: List[str] = example.get("setup_commands", [])
            success_condition: str | None = example.get("success_condition")
            ...
            return {"prompt": messages, "info": info, "answer": example["completion"]}
...
"""
inference:
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 vf-vllm --model deathbyknowledge/Qwen3-8B-Shell-SFT --tensor-parallel-size 2 --data-parallel-size 3

training:
CUDA_VISIBLE_DEVICES=6,7 accelerate launch --config-file configs/zero3.yaml --num-processes 2 verifiers/examples/shellm.py
"""
